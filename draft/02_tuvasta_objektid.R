...
predict_person_from_image <- function(x){
  ...
  # peata laadimine 15-60 sekundiks
  Sys.sleep(runif(n = 1, min = 15, max = 60))
...
detect_person_from_road <- function(x){
  ...
  if (detect_available_memory() > 800000) {
    ...
    rstudioapi::restartSession("source('02_tuvasta_objektid.R', echo = TRUE)")
...
map(1:500, detect_person_from_road)
